...
| All CSS id and class names must be in **camelCase**, with the exception that Nikita Smiichyk's initials (**ns**) must be included in lowercase followed by an underscore (_) in front of all id and class names. For example: `id="ns_productsTable"` and `class="ns_highlightedText"`.                                  |           |                |
